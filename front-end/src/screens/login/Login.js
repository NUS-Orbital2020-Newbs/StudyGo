...
import Spinner from "../../StudyGoSpinner";
...
export default function Login({ navigation }) {
  ...
  function handleLogin() {
    ...
    if (valid) {
      // if both the email and password are not empty
      ...
      setLoading(true);
      fetch(
        "https://fir-tut2-82e4f.firebaseapp.com/api/v1/login",
        requestOption
      )
        .then((res) => res.json())
        .then((data) => {
    ...
    return (
      <View style={LoginStyles.container2}>
        ...
        <View style={LoginStyles.whitePanel}>
          ...
          {emptyEmail && (
            <Text style={LoginStyles.wrongInputAlert}>Please enter email</Text>
          )}

          {/* password input */}
          <View
            style={[LoginStyles.input, emptyPassword && { marginBottom: 0 }]}
          >
            <View style={LoginStyles.inpIconContainer}>
